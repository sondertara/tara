...
import com.sondertara.common.id.UuidGenerator;
...
public class ProgressSource extends AbstractNameable {
    ...
    public ProgressSource(String eventDomain, String id, long expected, Object ref, ProgressTracer tracer) {
        Assert.notBlank(eventDomain, "eventDomain");
    ...
    public void update(long progress, long expected) {
        Valid.checkState(state == State.UPDATING, "not in updating: " + getName());

        if (progress > 0L) {
            this.progress = progress;
        }
        if (expected > 0L) {
            Valid.isTrue(expected >= this.progress);
            this.expected = expected;
        }
        if (logger.isDebugEnabled()) {
            logger.debug(this.toString());
    ...
    public String toString() {
        return StringUtils.format("progress trace: {}, current: {}, expected: {}, percent: {}, state: {}", getName(), getProgress(), getExpected(), percentAsString(), getState());
    }
