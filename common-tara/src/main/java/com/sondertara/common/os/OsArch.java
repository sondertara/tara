...
public enum OsArch implements CommonEnum<Integer> {
    /**
     *
     */
     ...
     *  ARM v8
     */
    AARCH64(3003, "aarch64", "64 位 ARM 架构", OsArchSeries.ARM, 64),
    ...
    public static OsArch findByName(String osArch) {
        final String theName = StringUtils.lowerCase(osArch);
        return StreamUtils.of(EnumSet.allOf(OsArch.class))

                .filter(arch -> arch.getName().contains(theName)).findFirst().get();
    }
