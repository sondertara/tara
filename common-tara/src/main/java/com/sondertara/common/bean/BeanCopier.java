...
class BeanCopier extends AbstractCopier {
    ...
    private static List<Copier> analyze(Class<?> sourceCls, Class<?> targetCls) {
        ...
        for (Field toField : targetFields) {
            ...
            if (toFieldGType instanceof ParameterizedType) {
                ...
                if (Set.class.isAssignableFrom(fieldCls)) {
                    copiers.add(new CollectionCopier(fromField, toField, Utils.nameOf(fromEtlTypes[0]),
                            Utils.nameOf(toEtlTypes[0]), true));
                } else if (Collection.class.isAssignableFrom(fieldCls)) {
                    copiers.add(new CollectionCopier(fromField, toField, Utils.nameOf(fromEtlTypes[0]),
                            Utils.nameOf(toEtlTypes[0]), false));
                } else if (Map.class.isAssignableFrom(fieldCls)) {
                    try {
                        if (!toEtlTypes[0].equals(fromEtlTypes[0]) && !Class.forName(Utils.nameOf(toEtlTypes[0]))
                                .isAssignableFrom(Class.forName(Utils.nameOf(fromEtlTypes[0])))) {
                            throw new BeanAnalysisException("Key types mismatch: " + fromField + " <-> " + toField);
                        }
                    } catch (ClassNotFoundException e) {
                        throw new BeanAnalysisException(e);
                    }
                    copiers.add(new MapCopier(fromField, toField, Utils.nameOf(fromEtlTypes[1]),
                            Utils.nameOf(toEtlTypes[1])));
                } else {
                    TypeConverter<?> converter = ConverterRegistry.find(fromField.getType().getName(),
                            toField.getType().getName());
                    if (converter != null) {
                        copiers.add(new SingleCopier(fromField, toField));
                    } else {
                        throw new BeanAnalysisException(
                                "Custom generic type requires converter, otherwise is not supported!");
                    }
                }
            } else {
        ...
        if (copiers.isEmpty()) {
            throw new BeanAnalysisException(
                    sourceCls.getName() + " & " + targetCls.getName() + " have no common fields to copy!");
        }
        return copiers;
    }
    ...
    public String toString() {
        ensureAnalyzed();
        return "BeanCopier{" + "fromField=`" + fromField + "`, toField=`" + toField + "`, fromCls=`" + fromCls
                + "`, toCls=`" + toCls + "`, converter=" + converter + ", copiers=" + join(copiers) + "}";
    }
