...
public final class PropertyUtils {
    ...
    public static PropertyDescriptor getPropertyDescriptorByNameOrThrow(Class<?> beanClass, String propertyName) {
        ...
        Assert.notNull(propertyDescriptor, () -> new IllegalArgumentException(String.format("Property '%s' not found for '%s'", propertyName, beanClass.getSimpleName())));
    ...
    public static void writeDirectly(Object destination, Field field, Object value) {
        Assert.notNull(destination, () -> new IllegalArgumentException("Destination must not be null"));
        try {
    ...
    public static <T> PropertyDescriptor getPropertyDescriptor(Class<T> beanClass, TypedPropertyGetter<T, ?> propertyGetter) {
        ...
        Assert.notNull(propertyDescriptor, () -> new IllegalArgumentException(String.format("Found no property for %s on %s", method, beanClass)));
        return propertyDescriptor;
    }
...
//		}
    ...
    private static <T> T createProxy(Class<T> beanClass, MethodCaptor methodCaptor) {
        ...
        try {
            ...
            throw new ReflectionException("Failed to create proxy on " + beanClass, e);
        }
    }
