...
public class Primitives {
    ...
    public static <T> Class<T> wrap(Class<T> type) {
        ...
        Class<T> wrapped = (Class<T>) PRIMITIVE_TO_WRAPPER_TYPE.get(Objects.requireNonNull(type));
    ...
    public static Class wrap(Type type) {
        if (isPrimitive(type)) {
            return PRIMITIVE_TO_WRAPPER_TYPE.get(Objects.requireNonNull(type));
        }
        return (Class) type;
    }
    ...
    public static <T> Class<T> unwrap(Class<T> type) {
        // cast is safe: long.class and Long.class are both of type Class<Long>
        @SuppressWarnings("unchecked")
        Class<T> unwrapped = (Class<T>) WRAPPER_TO_PRIMITIVE_TYPE.get(Objects.requireNonNull(type));
        return (unwrapped == null) ? type : unwrapped;
    }


    public static short sizeOf(Class<?> type) {
        Valid.isTrue(isPrimitive(type));
