...
import com.sondertara.common.convert.TypeStore;
...
public class TypeInfer {
    ...
    public static <T> T newInstanceInfer(List<Constructor<T>> constructors, Object[] params) {
        int index = -1;
        Class<?>[] sourceClasses = ArrayUtils.mapper(params, Class<?>[]::new, Object::getClass);
        List<Class<?>[]> targetClassesList = constructors.stream()
                .filter(m -> m.getParameterCount() == params.length)
                .peek(n->n.setAccessible(true))
        ...
        if (index == -1) {
            throw new IllegalStateException("could infer new instance, not found match constructor");
        }
        Class<?>[] targetClass = targetClassesList.get(index);
        Object[] inferParams = convertType(params, targetClass);
        return ReflectUtils.newInstance(constructors.get(index), inferParams);
    }
    ...
    public static <T> T invokeInfer(Object o, List<Method> methods, Object[] params) {
        ...
        if (index == -1) {
            throw new IllegalStateException("could infer invoke method, not found match method");
        }
        Class<?>[] targetClass = targetClassesList.get(index);
        Object[] inferParams = convertType(params, targetClass);
