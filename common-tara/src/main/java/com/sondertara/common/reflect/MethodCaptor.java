...
public class MethodCaptor {
    ...
    void capture(Method method) {
        ...
        Assert.isNull(existing, () -> new IllegalArgumentException(String.format("Method already captured: %s called twice?", existing)));
    }

    Method getCapturedMethod() {
        Method method = capturedMethod.get();
        Assert.notNull(method, () -> new IllegalArgumentException("Method could not be captured." +
