package com.sondertara.common.id.vm;
...
public class VMID implements Serializable {
    ...
    private static byte[] getHostAddress() {
        return (byte[]) AccessController.doPrivileged((PrivilegedAction<byte[]>) () -> {
            try {
    ...
    private static VMID create() {
        ...
        return new VMID(address, String.valueOf(JdkUtils.PID), new UID());
    }
}
