...
import com.sondertara.common.classpath.classloader.JarClassLoader;
...
public class ClassLoaderUtils {
     ...
     *
     * @param name          类名
     * @param classLoader   {@link ClassLoader}，{@code null}
     *                      则使用{@link #getClassLoader()}获取
     * @param isInitialized 是否初始化类（调用static模块内容和初始化static属性）
     * @return 类名对应的类
     */
    public static Class<?> loadClass(String name, ClassLoader classLoader, boolean isInitialized) {
        ...
        if (clazz == null) {
            ...
            clazz = CLASS_CACHE.computeIfAbsent(Pair.of(name, classLoader),
                    (key) -> doLoadClass(finalName, finalClassLoader, isInitialized));
        }
        return clazz;
    }
    ...
    public static Class<?> loadPrimitiveClass(String name) {
        ...
        if (StringUtils.isNotBlank(name)) {
            name = name.trim();
            if (name.length() <= PRIMITIVE_MAX_LENGTH) {
    ...
    private static Class<?> doLoadClass(String name, ClassLoader classLoader, boolean isInitialized) {
        ...
        } else if (name.startsWith(NON_PRIMITIVE_ARRAY_PREFIX) && name.endsWith(StringUtils.SEMICOLON)) {
