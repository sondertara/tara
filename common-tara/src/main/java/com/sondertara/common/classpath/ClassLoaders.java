...
import com.sondertara.common.classpath.classloader.ClassLoaderAccessor;
import com.sondertara.common.classpath.classloader.ExceptionIgnoringAccessor;
...
public class ClassLoaders {
    ...
    public static Class loadImplClass(@NonNull final String className, @Nullable ClassLoader classLoader, final Class superClass) throws ClassNotFoundException {
        ...
        if (!superClass.isAssignableFrom(clazz)) {
            final String error = "Class " + ReflectUtils.getFQNClassName(clazz) + " is not cast to " + ReflectUtils.getFQNClassName(superClass);
    ...
    public static <I, O> O doAction(@NonNull ClassLoader threadContextClassLoader, @NonNull Function<I, O> action, @Nullable I input) {
        Objects.requireNonNull(threadContextClassLoader);
    ...
    public static boolean hasClass(@NonNull String classFQN, @Nullable ClassLoader classLoader) {
        Objects.requireNonNull(classFQN, "the calss name is null or empty");
    ...
    public static Class<?> forName(String fqcn) throws ClassNotFoundException {
        ...
        if (clazz == null) {
            String msg = StringUtils.format("Unable to load class named [{}] from the thread context, current, or system/application ClassLoaders.  All heuristics have been exhausted.  Class could not be found.", fqcn);
     ...
     */
    private static final Map<Class<?>, Method> addURLMethodMap = new NonAbsentHashMap<Class<?>, Method>(new Function<Class<?>, Method>() {
        @Override
        public Method apply(Class<?> urlClassLoaderClass) {
            if (!URLClassLoader.class.isAssignableFrom(urlClassLoaderClass)) {
                return null;
            }

            Method addURLMethod = ReflectUtils.getAnyMethod(urlClassLoaderClass, "addURL", URL.class);
            if (addURLMethod != null) {
                ReflectUtils.makeAccessible(addURLMethod);
            }
            return addURLMethod;
        }
    });
    ...
    public static boolean addUrl(URLClassLoader urlClassLoader, URL jarUrl, boolean force) {
        ...
        if (addURLMethod != null) {
            try {
                ReflectUtils.invoke(addURLMethod, urlClassLoader, new Object[]{jarUrl}, true);
            } catch (Throwable ex) {
                return false;
            }
        }
    ...
    public static URL getJarUrl(Class<?> klass) {
        URL location = ReflectUtils.getCodeLocation(klass);
        return location;
    }
    ...
    public static JarFile getJarFile(Class<?> klass) {
        ...
        if (location != null) {
            ...
            try {
                ...
                Logger logger = Loggers.getLogger(ClassLoaders.class);
                logger.warn("Can't find the jar for class: {}", ReflectUtils.getFQNClassName(klass));
            }
        }
        return null;
    }
