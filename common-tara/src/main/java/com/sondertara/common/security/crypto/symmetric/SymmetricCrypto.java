...
package com.sondertara.common.security.crypto.symmetric;
...
import com.sondertara.common.security.crypto.CipherMode;
import com.sondertara.common.security.crypto.CryptoException;
import com.sondertara.common.security.crypto.JceCipher;
import com.sondertara.common.security.crypto.KeyUtils;
import com.sondertara.common.security.crypto.Padding;
import com.sondertara.common.security.crypto.openssl.OpenSSLSaltParser;
import com.sondertara.common.security.crypto.openssl.SaltMagic;
...
public class SymmetricCrypto implements SymmetricEncryptor, SymmetricDecryptor, Serializable {
    ...
    public void encrypt(final InputStream data, final OutputStream out, final boolean isClose) throws IORuntimeException {
        ...
        try {
            ...
            final long length = IOUtils.copy(data, cipherOutputStream);
    ...
    private SymmetricCrypto initParams(final String algorithm, AlgorithmParameterSpec paramsSpec) {
        if (null == paramsSpec) {
            ...
            // 随机IV
            if (StringUtils.startWithIgnoreCase(algorithm, "PBE")) {
                // 对于PBE算法使用随机数加盐
                if (null == iv) {
                    iv = RandomUtils.randomBytes(8);
                }
                paramsSpec = new PBEParameterSpec(iv, 100);
            } else if (StringUtils.startWithIgnoreCase(algorithm, "AES")) {
                if (null != iv) {
                    //AES使用Cipher默认的随机盐
    ...
    private JceCipher initMode(final CipherMode mode, final byte[] salt) {
        ...
        if (null != salt) {
            ...
            secretKey = KeyUtils.generateKey(algorithm, keyAndIV[0]);
            if (ArrayUtils.isNotEmpty(keyAndIV[1])) {
    ...
    private static void copyForZeroPadding(final CipherInputStream in, final OutputStream out, final int blockSize) throws IOException {
        int n = 1;
        if (IOUtils.DEFAULT_BUFFER_SIZE > blockSize) {
            n = Math.max(n, IOUtils.DEFAULT_BUFFER_SIZE / blockSize);
        }
        ...
        int preReadSize = 0;
        for (int readSize; (readSize = in.read(buffer)) != IOUtils.EOF; ) {
            if (isFirst) {
                isFirst = false;
            } else {
                // 将前一批数据写出
            ...
            ArrayUtils.copy(buffer, preBuffer, readSize);
