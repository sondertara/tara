...
public class PemUtil {
	...
	public static Key readPemKey(final InputStream keyStream) {
		...
		if (StringUtils.isNotBlank(type)) {
			...
			if (type.endsWith("EC PRIVATE KEY")) {
				try {
					// 尝试PKCS#8
					return KeyUtils.generatePrivateKey("EC", object.getContent());
				} catch (final Exception e) {
					// 尝试PKCS#1
					return KeyUtils.generatePrivateKey("EC", ECKeyUtil.createOpenSSHPrivateKeySpec(object.getContent()));
				}
			}
			...
			if (type.endsWith("EC PUBLIC KEY")) {
				try {
					// 尝试DER
					return KeyUtils.generatePublicKey("EC", object.getContent());
				} catch (final Exception ignore) {
					// 尝试PKCS#1
					return KeyUtils.generatePublicKey("EC", ECKeyUtil.createOpenSSHPublicKeySpec(object.getContent()));
				}
	...
	public static void writePemObject(final PemObjectGenerator pemObject, final OutputStream keyStream) {
		writePemObject(pemObject, IOUtils.getUtf8Writer(keyStream));
	}
