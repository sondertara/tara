...
public class SM4Utils {
     ...
     *               key=HexUtils.encodeHexStr(autoGenerateKey(DEFAULT_KEY_SIZE))
     * @param srcStr 待加密字符串
     * @return 返回16进制的加密字符串
     */
    public static String encryptByEcb(String srcStr, String key) {
        try {
            // 16进制字符串byte[]
            byte[] keyData = HexUtils.decode(key);
    ...
    public static String decryptEcb(String hexKey, String cipherText) {
        try {
            // hexString byte[]
            byte[] keyData = HexUtils.decode(hexKey);
            // hexString byte[]
            byte[] cipherData = HexUtils.decode(cipherText);
            // 解密
    ...
    public static void main(String[] args) {
        ...
        // 自动生成密钥
        String key = HexUtils.encodeHexStr(autoGenerateKey(DEFAULT_KEY_SIZE));
        // 加密
