...
package com.sondertara.common.security.crypto.openssl;
...
import com.sondertara.common.security.crypto.SpecUtils;
...
public class OpenSSLPBEOutputStream extends CipherOutputStream {
	...
	private static byte[] writeRandomHeader(final OutputStream out) throws IORuntimeException {
		final byte[] salt = RandomUtils.randomBytes(SaltMagic.SALT_LEN);
		try {
	...
	private static Cipher createEncryptCipher(final String algorithm, final byte[] salt,
											  final int iterationCount,
											  final char[] password) {
		final Cipher cipher = SecureUtils.createCipher(algorithm);
		try {
			cipher.init(Cipher.ENCRYPT_MODE,
				KeyUtils.generatePBEKey(algorithm, password),
				SpecUtils.createPBEParameterSpec(salt, iterationCount));
