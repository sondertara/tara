...
import com.sondertara.common.security.crypto.bc.ECKeyUtil;
import com.sondertara.common.security.crypto.bc.SmUtil;
...
public class KeyUtils {
    ...
    public static SecretKey generateDESKey(final String algorithm, final byte[] key) {
        ...
        if (null == key) {
            ...
            secretKey = generateKey(algorithm,
                    SpecUtils.createKeySpec(algorithm, key));
        }
        return secretKey;
    }
    ...
    public static SecretKey generatePBEKey(final String algorithm, char[] password) {
        ...
        if (null == password) {
            password = RandomUtils.randomString(32).toLowerCase().toCharArray();
        }
        return generateKey(algorithm, SpecUtils.createPBEKeySpec(password));
    }
    ...
    public static KeyPair getKeyPair(final String type, final InputStream in, final char[] password, final String alias) {
        final KeyStore keyStore = KeyStoreUtils.readKeyStore(type, in, password);
    ...
    public static String getAlgorithmAfterWith(String algorithm) {
        ...
        algorithm = getMainAlgorithm(algorithm);
        final int indexOfWith = StringUtils.lastIndexOfIgnoreCase(algorithm, "with");
        if (indexOfWith > 0) {
     ...
     *
     * @param in {@link InputStream} 如果想从文件读取.cer文件，使用 {@link FileUtils#getInputStream(File)} 读取
     * @return {@link KeyStore}
     * @since 4.5.2
     */
    public static PublicKey readPublicKeyFromCert(final InputStream in) {
        final Certificate certificate = CertUtils.readX509Certificate(in);
        if (null != certificate) {
