...
package com.sondertara.common.security.crypto.asymmetric;
...
import com.sondertara.common.security.crypto.SignUtils;
...
public class Sign extends BaseAsymmetric<Sign> {
	...
	public Sign(final String algorithm, final byte[] privateKey, final byte[] publicKey) {
		this(algorithm, //
				KeyUtils.generatePrivateKey(algorithm, privateKey), //
				KeyUtils.generatePublicKey(algorithm, publicKey)//
		);
	}
	...
	public Sign init(final String algorithm, final PrivateKey privateKey, final PublicKey publicKey) {
		signature = SignUtils.createSignature(algorithm);
	...
	/**
	 * 生成签名，使用默认缓存大小，见 {@link IOUtils#DEFAULT_BUFFER_SIZE}
	 *
	...
	public String digestHex(final InputStream data, final int bufferLength) {
		return HexUtils.encodeHexStr(sign(data, bufferLength));
	}
