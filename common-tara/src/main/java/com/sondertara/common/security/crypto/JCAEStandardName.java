...
public enum JCAEStandardName {
    ...
    JCAEStandardName() {
        Algorithm algorithm = ReflectUtils.getDeclaredField(JCAEStandardName.class,name()).getAnnotation(Algorithm.class);
        if (algorithm != null) {
    ...
    public Class[] getScenarios() {
        return ArrayUtils.copy(this.scenarios);
    }

    public static List<JCAEStandardName> findAlgorithms(@NonNull final Class scenario) {
        Assert.notNull(scenario);
        return StreamUtils.of(EnumSet.allOf(JCAEStandardName.class))
                .filter(new Predicate<JCAEStandardName>() {
                    @Override
                    public boolean test(JCAEStandardName sn) {
                        return sn.isAlgorithm() && CollectionUtils.contains(sn.scenarios, scenario);
                    }
                })
