...
public class Vertex<T> extends AbstractNameable {
    private List<Edge<T>> incomingEdges = new ArrayList<>();
    private List<Edge<T>> outgoingEdges = new ArrayList<>();
    @Nullable
    private T data;
    ...
    public List<Vertex<T>> getIncomingVertices() {
        ...
        return StreamUtils.of(edges).map(Edge::getFrom).collect(Collectors.toList());
    }
    


    public List<String> getIncomingVertexNames() {
        return StreamUtils.of(getIncomingVertices()).map(new Function<Vertex<T>, String>() {
            @Override
    ...
    public List<Vertex<T>> getOutgoingVertices() {
        ...
        return StreamUtils.of(edges).map(new Function<Edge<T>, Vertex<T>>() {
            @Override
    ...
    public List<String> getOutgoingVertexNames() {
        return StreamUtils.of(getOutgoingVertices()).map(new Function<Vertex<T>, String>() {
            @Override
            public String apply(Vertex<T> vertex) {
                return vertex.getName();
            }
    ...
    public boolean equals(Object o) {
        ...
        return ObjectUtils.equals(this.name, vertex.name);
    }
    ...
    public String toString() {
        ...
        String in = StringUtils.join(",", StreamUtils.of(incomingEdges).map(new Function<Edge<T>, String>() {
            @Override
            public String apply(Edge<T> e) {
                return StringUtils.format("{from: {}, edge_label: {}, weight:{}}", e.getFrom().getName(), e.getLabel(), e.getWeight());
            }
        }).collect(Collectors.toList()));
        tmp.append(in);
        tmp.append("], out:[");

        String out = StringUtils.join(",", StreamUtils.of(outgoingEdges).map(new Function<Edge<T>, String>() {
            @Override
            public String apply(Edge<T> e) {
                return StringUtils.format("{to: {}, edge_label: {}, weight:{}}", e.getTo().getName(), e.getLabel(), e.getWeight());
            }
        }).collect(Collectors.toList()));
        tmp.append(out);
        tmp.append(']');
