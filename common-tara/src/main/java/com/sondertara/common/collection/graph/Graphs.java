...
import com.sondertara.common.collection.graph.traverser.BreadthFirstGraphTraverser;
import com.sondertara.common.collection.graph.traverser.DeepFirstGraphTraverser;
import com.sondertara.common.collection.graph.traverser.TreeGraphTraverser;
import com.sondertara.common.comparator.IntegerComparator;
...
public class Graphs {
    ...
    private static final Function<String, VisitStatus> NOT_VISITED_SUPPLIER = new Function<String, VisitStatus>() {
        @Override
        public VisitStatus apply(String name) {
            return VisitStatus.NOT_VISITED;
        }
    };

    public static Map<String, VisitStatus> newVisitStatusMap() {
        return CollectionUtils.emptyNonAbsentHashMap(NOT_VISITED_SUPPLIER);
    }

    public static VisitStatus getVisitStatus(Map<String, VisitStatus> statusMap, String name) {
        return CollectionUtils.wrapAsNonAbsentMap(statusMap, NOT_VISITED_SUPPLIER).get(name);
    }
    ...
    public static <T> void traverse(final GraphTraverser traverser, final Graph<T> graph, final VertexConsumer<T> consumer) {
        ...
        CollectionUtils.forEach(graph.getVertices(), new Consumer<Vertex<T>>() {
            @Override
    ...
    public static <T> List<Vertex<T>> tdfsSort(final Graph<T> graph) {
        ...
        VertexConsumer<T> consumer = new VertexConsumer<T>() {
            ...
            public void accept(Graph<T> graph, Vertex<T> vertex, Edge<T> edge) {
                ...
                int minToIndex = -1;
                List<Integer> indexes = StreamUtils.of(vertex.getOutgoingVertices())
                ...
                if (ObjectUtils.isNotEmpty(indexes)) {
                    if (indexes.size() == 1) {
                        minToIndex = indexes.get(0);
                    } else {
                        minToIndex = StreamUtils.of(indexes).min(new IntegerComparator()).get();
                    }
                }
