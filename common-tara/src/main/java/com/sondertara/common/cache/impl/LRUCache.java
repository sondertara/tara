...
import com.sondertara.common.collection.FixedLinkedHashMap;
...
public class LRUCache<K, V> extends ReentrantCache<K, V> {
    ...
    public LRUCache(int capacity, final long timeout) {
        super(capacity < Integer.MAX_VALUE && capacity > 0 ? capacity : Integer.MAX_VALUE);
        this.setExpireAfterAccess(timeout);
        //链表key按照访问顺序排序，调用get方法后，会将这次访问的元素移至头部
        final FixedLinkedHashMap<Mutable<K>, CacheObj<K, V>> fixedLinkedHashMap = new FixedLinkedHashMap<>(this.capacity);
        fixedLinkedHashMap.setEvictionListener((k, v) -> {
            if (null != listener) {
                listener.onRemove(k.get(), v.getValue());
            }
        });
