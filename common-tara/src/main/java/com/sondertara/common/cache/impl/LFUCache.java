...
public class LFUCache<K, V> extends StampedCache<K, V> {
	...
	public LFUCache(int capacity, long timeout) {
		super(capacity < Integer.MAX_VALUE && capacity > 0 ? capacity : Integer.MAX_VALUE,timeout);
		cacheMap = new HashMap<>(capacity < Integer.MAX_VALUE && capacity > 0 ? capacity+1 : Integer.MAX_VALUE, 1.0f);
	}
