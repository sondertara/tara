...
import com.sondertara.common.cache.impl.CacheObj;
...
public interface Cache<K, V> extends Serializable {
	 ...
	 * @return 值对象
	 */
	V get(K key, boolean isUpdateLastAccess, long timeout, Supplier<V> supplier);
	 ...
	 */
	default void put(@NonNull K key, @Nullable V value, long ttl, TimeUnit timeUnit){
		put(key,value,timeUnit.toMillis(ttl));
	}
	 ...
	 * @return the values
	 */
	Map<K, V> get(@NonNull Iterable<K> keys);
