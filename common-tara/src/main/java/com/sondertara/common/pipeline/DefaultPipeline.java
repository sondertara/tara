...
public class DefaultPipeline<T> implements Pipeline<T> {
    ...
    public void inbound() throws Throwable {
        Assert.isFalse(targetHolder.isNull(), "target is null");
        getHead().inbound();
    }
    ...
    public void outbound() throws Throwable {
        Assert.isFalse(this.targetHolder.isNull(), "target is null");
        Objects.requireNonNull(current, "current handler context is null");
        current.outbound();
    }
    ...
    public void setHeadHandler(Handler headHandler) {
        Assert.isTrue(head != null && !head.isSkiped() && !head.isInbounded() && !head.isOutbounded());
    ...
    public void setTailHandler(Handler tailHandler) {
        Assert.isTrue(tail != null && !tail.isSkiped() && !tail.isInbounded() && !tail.isOutbounded());
