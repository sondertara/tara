...
import com.sondertara.common.retry.backoff.ExponentialBackoffPolicy;
...
public class RetryConfig {
    ...
    public RetryConfig(int maxAttempts,
                       ...
                       TimeUnit timeUnit,
                       BackoffPolicy backoffPolicy) {
        this.maxAttempts = maxAttempts<0 ? -1 : Math.max(1,maxAttempts);
        ...
        if (this.timeout>0){
            Valid.isTrue(this.timeout>this.delay, StringUtils.format("illegal args, timeout: {}, delay: {}", this.timeout, this.delay));
        }
    }
    ...
    public RetryConfig setJitter(float jitter) {
        Valid.isTrue(jitter >= 0.0f && jitter < 1.0f,
