...
public class DelimiterBasedReadableByteChannel implements ReadableByteChannel, Iterable<byte[]> {
    ...
    private void setDelimiters(String... delimiters) {
        boolean hasLineDelimiter = CollectionUtils.anyMatch(new Predicate<String>() {
            @Override
            public boolean test(String delimiter) {
                return LineDelimiter.isLineDelimiter(delimiter);
            }
        }, delimiters);
        List<String> dels = StreamUtils.of(delimiters).collect(Collectors.toList());
        if (hasLineDelimiter) {
            dels.addAll(LineDelimiter.supportedLineDelimiterStrings());
        }

        List<ByteBuffer> ds = dels.stream()
                ...
                .map(new Function<String, ByteBuffer>() {
                    ...
                    public ByteBuffer apply(String delimiter) {
                        return ByteBuffer.wrap(delimiter.getBytes(StandardCharsets.UTF_8));
                    }
                })
                .collect(Collectors.toList());
         Assert.isTrue(ObjectUtils.isNotEmpty(ds), "delimiters is null or empty");
        this.delimiters = ds;
    ...
    private int fill() throws IOException {
        ...
        if (buf == null) {
            ...
            if (buf.remaining() < Math.max(this.maxLengthOfDelimiters, 1)) {
        ...
        if (delimiters.size() > 1) {
            while (buf.hasRemaining() && buf.remaining() >= this.minLengthOfDelimiters) {
                ...
                for (ByteBuffer del : delimiters) {
                    ...
                    buf.get(tmp);
                    buf.reset();
                    if (ObjectUtils.equals(tmp, del.array())) {
                        delimiter = del;
                        break;
                    }
                }
