...
public final class OsFileSystem {
    ...
    OsFileSystem(final int blockSize,
        ...
        this.reservedFileNames = Lists.newArrayList(Objects.requireNonNull(reservedFileNames, "reservedFileNames"));
        ...
        pathSeparators.add("/");
        this.pathSeparators = CollectionUtils.toArray(pathSeparators, String[].class);
    }
    ...
    public boolean isLegalFileName(final CharSequence filename) {
        int length = ObjectUtils.length(filename);
        ...
        for (int i = 0; i < filename.length(); i++) {
            ...
            if (CollectionUtils.contains(illegalFileNameChars, c)) {
                return false;
            }
        }
        return true;
    ...
    protected CharSequence trimExtension(final CharSequence cs) {
        final int index = StringUtils.lastIndexOf(cs, ".");
    ...
    public String normalizeSeparators(final String path) {
        return StringUtils.replaceChars(path, pathSeparatorOther, pathSeparator);
    }
    ...
    private boolean isIllegalFileNameChar(final char c) {
        return StreamUtils.of(illegalFileNameChars).collect(Collectors.toSet()).contains(c);
    }
