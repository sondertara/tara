package com.sondertara.common.io.unicode;
...
public class Utf8s {
     ...
     * e.g. ：\x{we}
     */
    public static final Regexp ESCAPED_CHAR_REGEXP = RegexUtils.createRegexp("(?:(?:\\\\0)[0-3][0-7]{2})|(?:(?:\\\\0)[0-7]{1,2})|(?:(?:\\\\x)[0-9a-fA-F]{2})|(?:(?:\\\\u)[0-9a-fA-F]{4})");
    public static final Regexp X16_CHAR_REGEXP= RegexUtils.createRegexp("(?:(?:\\\\x)[0-9a-fA-F]{2})");
    ...
    public static char hexToChar(String hex, boolean valid) {
        if (valid) {
             Assert.isTrue(RegexUtils.match(ESCAPED_CHAR_REGEXP, hex), "illegal hex: {}", hex);
        }
        if (hex.startsWith("\\0")) {
            // unicode
            hex = hex.substring(2);
