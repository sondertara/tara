...
public class NamedRegexp implements Regexp, Serializable {
    ...
    private StringBuilder replaceGroupNameWithIndex(StringBuilder input, Pattern pattern, String prefix) {
        ...
        while (m.find()) {
            ...
            int index = Groups.indexOf(groupInfo,m.group(Groups.INDEX_GROUP_NAME));
            if (index >= 0) {
                index++;
            } else {
                throw new PatternSyntaxException("unknown group name", input.toString(), m.start(Groups.INDEX_GROUP_NAME));
            }
    ...
    private Pattern buildStandardPattern(String namedPattern, Integer flags) {
        ...
        s = replace(s, Groups.NAMED_GROUP_PATTERN, "(");
        s = replaceGroupNameWithIndex(s, Groups.BACKREF_NAMED_GROUP_PATTERN, "\\");
        ...
        if (a != null && b != null) {
            if (a.isEmpty() && b.isEmpty()) {
                ...
                for (Map.Entry<String, List<Groups.GroupCoordinate>> entry : a.entrySet()) {
                    ...
                    List<Groups.GroupCoordinate> thisList = entry.getValue();
