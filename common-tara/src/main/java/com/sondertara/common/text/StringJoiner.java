...
public final class StringJoiner implements Appendable {
    ...
    public StringJoiner append(Object[] newElements) {
        return append(Lists.asList(newElements));
    }
    ...
    public StringJoiner append(Collection newElements, Predicate2<Integer, ?> predicate) {
        BiConsumer<Integer, Object> consumer = (key, newElement) -> {
            if (ObjectUtils.isEmpty(newElement)) {
                add(emptyValue);
            } else {
        ...
        CollectionUtils.forEach(newElements, p, consumer);
        return this;
    }
