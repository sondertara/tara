...
/**
 * Singleton initializer class that uses an internal supplier to supply the
 * singleton instance. The supplier
 * originally checks whether the instanceSupplier
 * has been initialized or not, but after initialization the instance supplier
 * is changed to avoid extra logic
 * execution.
 * 
 * @author huangxiaohu
 */
public final class Singleton<R> implements Supplier<R> {
    ...
    public static <T, U, R> Singleton<R> fromLazy(final BiFunction<T, U, R> original, Supplier<T> arg0Supplier,
