package com.sondertara.common.net.cidr;
...
public final class CidrAddress implements Serializable, Comparable<CidrAddress> {
    ...
    public static CidrAddress create(InetAddress networkAddress, int netmaskBits) {
        Assert.notNull(networkAddress, "networkAddress");
        Valid.isTrue(netmaskBits >= 0, "the min value of netmaskBits is 0 ");
        int scopeId = Nets.getScopeId(networkAddress);
        if (networkAddress instanceof Inet4Address) {
            Valid.isTrue(netmaskBits <= 32, "the max value of netmaskBits is 32 for IPV4");
            if (netmaskBits == 0) {
                return INET4_ANY_CIDR;
            }
        } else if (networkAddress instanceof Inet6Address) {
            Valid.isTrue(netmaskBits <= 128, "the max value of netmaskBits is 128 for IPV6");
            if (netmaskBits == 0 && scopeId == 0) {
                return INET6_ANY_CIDR;
            }
        } else {
            throw new RuntimeException(StringUtils.format("illegal network address: {}", networkAddress));
        }
    ...
    static CidrAddress create(byte[] addressBytes, int netmaskBits, boolean clone) {
         Assert.notNull(addressBytes, "networkAddress");
        Valid.isTrue(netmaskBits >= 0, "the min value of netmaskBits is 0 ");
        ...
        if (length == 4) {
            ...
            throw new RuntimeException(StringUtils.format("illegal network address length: {}", length));
        }
    ...
    public int compareAddressBytesTo(final byte[] otherBytes, final int otherNetmaskBits, final int scopeId) {
         Assert.notNull(otherBytes, "bytes");
        final int otherLength = otherBytes.length;
        if (otherLength != 4 && otherLength != 16) {
            throw new RuntimeException(StringUtils.format("illegal network address length: {}", otherLength));
        }
        // IPv4 before IPv6
        final byte[] cachedBytes = this.cachedBytes;
