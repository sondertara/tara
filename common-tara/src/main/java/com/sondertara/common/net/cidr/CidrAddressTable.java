...
public final class CidrAddressTable<T> implements Iterable<CidrAddressTable.Mapping<T>>, Cloneable {
    ...
    public T put(CidrAddress block, T value) {
        Valid.notNull(block, "block");
        Valid.notNull(value, "value");
    ...
    private T doPut(final CidrAddress block, final T expect, final T update, final boolean putIfAbsent, final boolean putIfPresent) {
        ...
        do {
            ...
            if (expect != null) {
                matchesExpected = ObjectUtils.equals(expect, existing);
                if (!matchesExpected) {
                    return existing;
                }
            } else {
    ...
    */
    public String toString() {
        ...
        for (final Mapping<T> mapping : mappings) {
            b.append(IOUtils.LINE_SEPARATOR).append('\t').append(mapping.range);
