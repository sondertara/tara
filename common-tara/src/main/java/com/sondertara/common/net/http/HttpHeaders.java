...
import com.sondertara.common.annotation.NotThreadSafe;
...
import com.sondertara.common.collection.multivalue.MultiValueMapAdapter;
import com.sondertara.common.datetime.LocalDateTimeUtils;
import com.sondertara.common.function.Predicate2;
import com.sondertara.common.net.NetworkAddress;
import com.sondertara.common.net.mime.MediaType;
...
public class HttpHeaders implements MultiValueMap<String, String>, Serializable {
     ...
     * @see <a href="https://tools.ietf.org/html/rfc7232#section-2.3">Section 2.3 of RFC 7232</a>
     */
    private static final Regexp ETAG_HEADER_VALUE_PATTERN = RegexUtils.compile("\\*|\\s*((W\\/)?(\"[^\"]*\"))\\s*,?");
    ...
    */
    ...
    public HttpHeaders(MultiValueMap<String, String> headers) {
        Objects.requireNonNull(headers, "MultiValueMap must not be null");
    ...
    public static String formatHeaders(MultiValueMap<String, String> headers) {
        return "[" + StringUtils.join(", ", headers.entrySet().stream()
                .map(entry -> {
                    Collection<String> values = entry.getValue();
                    return entry.getKey() + ":" + (values.size() == 1 ?
                            "\"" + Lists.asList(values).get(0) + "\"" :
                            StringUtils.join(", ", StreamUtils.of(values)
     ...
     *                 sequence. Defaults to {@linkplain StandardCharsets#ISO_8859_1}.
    ...
    public static String encodeBasicAuth(String username, String password, @Nullable Charset charset) {
        Objects.requireNonNull(username, "Username must not be null");
         Assert.isTrue(!username.contains(":"), "Username must not contain a colon");
        ...
        return StringUtils.newString(encodedBytes, charset);
    }
    ...
    public List<String> getOrEmpty(Object headerName) {
        ...
        return (values != null ? Lists.asList(values) : Collections.emptyList());
    }
    ...
    public List<MediaType> getAccept() {
        return MediaType.parseMediaTypes(Lists.asList(get(ACCEPT)));
    }
    ...
    public void setAccessControlAllowMethods(List<HttpMethod> allowedMethods) {
        set(ACCESS_CONTROL_ALLOW_METHODS, StringUtils.join(",", allowedMethods));
    }
    ...
    public Locale getContentLanguage() {
        String language =getValuesAsList(CONTENT_LANGUAGE).stream()
    ...
    public void setExpires(long time) {
        set(EXPIRES, LocalDateTimeUtils.getDateTimeSecondStr(time));
    }
    ...
    public List<String> getValuesAsList(String headerName) {
        ...
        if (values != null) {
            ...
            for (String value : values) {
                if (value != null) {
                    Collections.addAll(result, StringUtils.split(value, ","));
                }
            }
            return result;
        }
    ...
    public String getFirstHeader(Predicate2<String, String> predicate, String... headerNames) {
        return StreamUtils.of(headerNames).filter(new Predicate<String>() {
            @Override
            public boolean test(String s) {
                return predicate.test(s,getFirstHeader(s));
            }
        }).findFirst().orElse(null);
    }
