package com.sondertara.common.datetime.format;
...
public class FastDateParser implements DateParser, Serializable {
    ...
    public Date parse(String source) {
        ...
        if (date == null) {
            ...
            if (locale.equals(JAPANESE_IMPERIAL)) {
                throw new IllegalArgumentException("(the " + locale + " locale does not support dates before 1868 AD)\n" +
                        "unParse date: \"" + source + " " + pp.getErrorIndex());
            }
            throw new IllegalArgumentException("unparsed date: " + source + " " + pp.getErrorIndex());
        }
        return date;
    }
    ...
    public Date parse(String source, ParsePosition pos) {
        ...
        cal.set(Calendar.YEAR, this.startYear + this.century);
        cal.getTime();
    ...
    private static StringBuilder simpleQuote(StringBuilder sb, String value) {
        ...
        if (sb.charAt(sb.length() - 1) == CharUtils.DOT) {
            sb.append('?');
        }
        return sb;
    }
    ...
    private static Map<String, Integer> appendDisplayNames(Calendar cal, Locale locale, int field, StringBuilder regex) {
        ...
        Map<String, Integer> values = new HashMap<>(displayNames.size());
    ...
    private Strategy getLocaleSpecificStrategy(int field, Calendar definingCalendar) {
        ...
        if (strategy == null) {
            strategy = field == Calendar.ZONE_OFFSET
                    ...
                    : new CaseInsensitiveTextStrategy(definingCalendar, locale, field);
    ...
    private abstract static class PatternStrategy implements Strategy {
    ...
    private static class CaseInsensitiveTextStrategy extends PatternStrategy {
        ...
        CaseInsensitiveTextStrategy(final Calendar definingCalendar, final Locale locale, int field) {
            this.field = field;
            this.locale = locale;
    ...
    private static class TimeZoneStrategy extends PatternStrategy {
        ...
        TimeZoneStrategy(final Calendar calendar, Locale locale) {
            this.locale = locale;
            StringBuilder sb = new StringBuilder();
            ...
            for (String[] zoneNames : zones) {
                ...
                TzInfo standard = new TzInfo(calendar.getTimeZone(), false);
                TzInfo tzInfo = standard;
                for (int i = 1; i < zoneNames.length; ++i) {
                    switch (i) {
                        case 3:
                            tzInfo = new TzInfo(calendar.getTimeZone(), true);
                            break;
                        case 5:
                            tzInfo = standard;
                            break;
