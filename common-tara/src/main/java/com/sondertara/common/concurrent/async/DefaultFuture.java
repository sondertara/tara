package com.sondertara.common.concurrent.async;
...
public class DefaultFuture<V> extends AbstractFuture<V> implements Callable<V>, Runnable {
    ...
    private final List<GenericFutureListener<? extends GenericFuture<? super V>>> listeners = new ArrayList<>();
    ...
    public DefaultFuture(Callable<V> task) {
    ...
    public GenericFuture<V> removeListeners(GenericFutureListener<? extends GenericFuture<? super V>>... listeners) {
        CollectionUtils.removeAll(this.listeners, Lists.asList(listeners));
        return this;
    }
    ...
    private boolean await(long timeoutMillis, boolean uninterruptible) throws InterruptedException {
        ...
        if(completed) {
            ...
            this.listeners.forEach(listener -> listener.operationComplete(_this));
        }
        return completed;
    }
