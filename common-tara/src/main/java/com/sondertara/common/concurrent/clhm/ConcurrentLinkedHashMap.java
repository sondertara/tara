 ...
 */
package com.sondertara.common.concurrent.clhm;


import com.sondertara.common.annotation.GuardedBy;
import com.sondertara.common.annotation.Immutable;
import com.sondertara.common.annotation.ThreadSafe;
import com.sondertara.common.collection.EvictionListener;
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V>
  ...
  V compute(final K key, final Function<? super K, ? extends V> mappingFunction, boolean onlyIfAbsent) {
    ...
    final Holder<Node<K, V>> objectHolder = new Holder<>();
    ...
    for (;;) {
      Function<K, Node<K, V>> f = k -> {
        ...
        objectHolder.set(node);

        return node;
      };
      Node<K, V> prior = data.computeIfAbsent(key, f);

      Node<K, V> node = objectHolder.get();
