...
final class JarURLConnection extends java.net.JarURLConnection {
        ...
        while ((separator = spec.indexOf(SEPARATOR, index)) > 0) {
            ...
            if (jarEntry == null) {
                return JarURLConnection.notFound(connectionJarFile, entryName,
                        (connectionJarFile != jarFile) ? new ConnectedJarFileCloseAction(connectionJarFile) : null);
            }
    ...
    static class ConnectedJarFileCloseAction implements CloseAction {
        private JarFile jarFile;

        ConnectedJarFileCloseAction(JarFile jarFile) {
            this.jarFile = jarFile;
        }
