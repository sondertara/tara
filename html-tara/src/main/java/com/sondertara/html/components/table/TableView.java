...
import com.sondertara.common.collection.table.RowKeyTable;
...
public class TableView extends BaseComponent {
    ...
    @SuppressWarnings("UnstableApiUsage")
    private RowKeyTable<Integer, String, TableCell> initTable(final Map<String, TableCol> colMap) {
        List<String> colNames = columns.stream().map(TableCol::getDataIndex).collect(Collectors.toList());
        RowKeyTable<Integer, String, TableCell> table = new RowKeyTable<>(true);
        ...
        for (String s : table.columnKeySet()) {
            ...
            Map<Integer, TableCell> cellMap = table.getColumn(s);
            int rowSpan = 1;
